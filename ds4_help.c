...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    if (full) {
        if (tool != DS4_HELP_BENCH) {
            ...
            opt(fp, c, "--dspark FILE", "Validate an official DSpark drafter GGUF sidecar. Runtime is not enabled yet.");
        }
...
static void print_cli_diagnostics(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--dspark-probe", "With --dspark, run target-state capture plus diagnostic sidecar logits/confidence.");
