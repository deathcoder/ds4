...
typedef struct rust_star_metal_prefill_layer2_attention_weights {
    ...
    rust_star_metal_prefill_kvnorm_weights layer39_kvnorm;
    uint64_t layer39_q_b_offset, layer39_q_b_bytes;
    rust_star_metal_prefill_compressor_weights layer39_compressor;
    uint64_t layer39_attn_sinks_offset, layer39_attn_sinks_bytes;
    uint64_t layer39_attn_output_a_offset, layer39_attn_output_a_bytes;
    uint64_t layer39_attn_output_b_offset, layer39_attn_output_b_bytes;
    rust_star_metal_prefill_ffn_weights layer39_ffn;
    rust_star_metal_prefill_kvnorm_weights layer40_kvnorm;
    uint64_t layer40_q_b_offset, layer40_q_b_bytes;
    rust_star_metal_prefill_compressor_weights layer40_compressor;
    uint64_t layer40_attn_sinks_offset, layer40_attn_sinks_bytes;
    uint64_t layer40_attn_output_a_offset, layer40_attn_output_a_bytes;
    uint64_t layer40_attn_output_b_offset, layer40_attn_output_b_bytes;
    rust_star_metal_prefill_ffn_weights layer40_ffn;
    rust_star_metal_prefill_kvnorm_weights layer41_kvnorm;
    uint64_t layer41_q_b_offset, layer41_q_b_bytes;
    rust_star_metal_prefill_compressor_weights layer41_compressor;
    uint64_t layer41_attn_sinks_offset, layer41_attn_sinks_bytes;
    uint64_t layer41_attn_output_a_offset, layer41_attn_output_a_bytes;
    uint64_t layer41_attn_output_b_offset, layer41_attn_output_b_bytes;
    rust_star_metal_prefill_ffn_weights layer41_ffn;
    rust_star_metal_prefill_kvnorm_weights layer42_kvnorm;
    uint64_t layer42_q_b_offset, layer42_q_b_bytes;
    rust_star_metal_prefill_compressor_weights layer42_compressor;
    uint64_t layer42_attn_sinks_offset, layer42_attn_sinks_bytes;
    uint64_t layer42_attn_output_a_offset, layer42_attn_output_a_bytes;
    uint64_t layer42_attn_output_b_offset, layer42_attn_output_b_bytes;
    rust_star_metal_prefill_ffn_weights layer42_ffn;
...
int rust_star_metal_run_prefill_layer2_attention(
    ...
    float *layer39_hc_attn_pre_final_tile,
    float *layer39_attn_norm_final_tile,
    float *layer39_q_lora_final_tile,
    float *layer39_q_lora_norm_final_tile,
    float *layer39_kv_raw_final_tile,
    float *layer39_kv_norm_final_tile,
    float *layer39_q_raw_final_tile,
    float *layer39_q_cur_final_tile,
    float *layer39_kv_rope_final_tile,
    float *layer39_kv_cur_final_tile,
    float *layer39_attn_compressed,
    float *layer39_attn_state_kv,
    int32_t *layer39_attn_state_score,
    float *layer39_kqv_out_row0,
    float *layer39_kqv_back_row0,
    float *layer39_attn_low_row0,
    float *layer39_attention_output,
    float *layer39_after_attention_hc,
    float *layer39_after_ffn_hc,
    float *layer39_ffn_cur_final_tile,
    float *layer39_ffn_norm_final_tile,
    int32_t *layer39_router_selected_final_tile,
    float *layer39_router_weights_final_tile,
    float *layer39_routed_out_final_tile,
    float *layer39_shared_out_final_tile,
    float *layer40_hc_attn_pre_final_tile,
    float *layer40_attn_norm_final_tile,
    float *layer40_q_lora_final_tile,
    float *layer40_q_lora_norm_final_tile,
    float *layer40_kv_raw_final_tile,
    float *layer40_kv_norm_final_tile,
    float *layer40_q_raw_final_tile,
    float *layer40_q_cur_final_tile,
    float *layer40_kv_rope_final_tile,
    float *layer40_kv_cur_final_tile,
    float *layer40_attn_compressed,
    float *layer40_attn_state_kv,
    int32_t *layer40_attn_state_score,
    float *layer40_indexer_compressed,
    float *layer40_indexer_state_kv,
    int32_t *layer40_indexer_state_score,
    float *layer40_kqv_out_row0,
    float *layer40_kqv_back_row0,
    float *layer40_attn_low_row0,
    float *layer40_attention_output,
    float *layer40_after_attention_hc,
    float *layer40_after_ffn_hc,
    float *layer40_ffn_cur_final_tile,
    float *layer40_ffn_norm_final_tile,
    int32_t *layer40_router_selected_final_tile,
    float *layer40_router_weights_final_tile,
    float *layer40_routed_out_final_tile,
    float *layer40_shared_out_final_tile,
    float *layer41_hc_attn_pre_final_tile,
    float *layer41_attn_norm_final_tile,
    float *layer41_q_lora_final_tile,
    float *layer41_q_lora_norm_final_tile,
    float *layer41_kv_raw_final_tile,
    float *layer41_kv_norm_final_tile,
    float *layer41_q_raw_final_tile,
    float *layer41_q_cur_final_tile,
    float *layer41_kv_rope_final_tile,
    float *layer41_kv_cur_final_tile,
    float *layer41_attn_compressed,
    float *layer41_attn_state_kv,
    int32_t *layer41_attn_state_score,
    float *layer41_kqv_out_row0,
    float *layer41_kqv_back_row0,
    float *layer41_attn_low_row0,
    float *layer41_attention_output,
    float *layer41_after_attention_hc,
    float *layer41_after_ffn_hc,
    float *layer41_ffn_cur_final_tile,
    float *layer41_ffn_norm_final_tile,
    int32_t *layer41_router_selected_final_tile,
    float *layer41_router_weights_final_tile,
    float *layer41_routed_out_final_tile,
    float *layer41_shared_out_final_tile,
    float *layer42_hc_attn_pre_final_tile,
    float *layer42_attn_norm_final_tile,
    float *layer42_q_lora_final_tile,
    float *layer42_q_lora_norm_final_tile,
    float *layer42_kv_raw_final_tile,
    float *layer42_kv_norm_final_tile,
    float *layer42_q_raw_final_tile,
    float *layer42_q_cur_final_tile,
    float *layer42_kv_rope_final_tile,
    float *layer42_kv_cur_final_tile,
    float *layer42_attn_compressed,
    float *layer42_attn_state_kv,
    int32_t *layer42_attn_state_score,
    float *layer42_indexer_compressed,
    float *layer42_indexer_state_kv,
    int32_t *layer42_indexer_state_score,
    float *layer42_kqv_out_row0,
    float *layer42_kqv_back_row0,
    float *layer42_attn_low_row0,
    float *layer42_attention_output,
    float *layer42_after_attention_hc,
    float *layer42_after_ffn_hc,
    float *layer42_ffn_cur_final_tile,
    float *layer42_ffn_norm_final_tile,
    int32_t *layer42_router_selected_final_tile,
...
/* Publishes exact layer-9 prefix/continuation HC and retained layer-10 state
 * through the generic even-layer ratio-4 executor. */
int rust_star_metal_begin_prefill_layer10_rebuild(
    void *context,
    char *error,
    size_t error_bytes);

/* Preserves exact layer-10 prefix HC before the generic even-layer slot is
 * reused for its continuation. */
int rust_star_metal_retain_prefill_layer10_prefix_hc(
    void *context,
    char *error,
    size_t error_bytes);

/* Retains buffers grown by the generic even-layer executor as layer-10 state. */
int rust_star_metal_finish_prefill_layer10_rebuild(
    void *context,
    char *error,
    size_t error_bytes);

/* Publishes exact layer-10 prefix HC and retained layer-11 compressor state
 * through the generic odd-layer ratio-128 executor. */
int rust_star_metal_begin_prefill_layer11_prefix_rebuild(
    void *context,
    char *error,
    size_t error_bytes);

/* Preserves exact layer-11 prefix state and binds its continuation to exact
 * retained layer-10 continuation HC. */
int rust_star_metal_begin_prefill_layer11_continuation(
    void *context,
    char *error,
    size_t error_bytes);

/* Retains buffers grown by the generic odd-layer executor as layer-11 state. */
int rust_star_metal_finish_prefill_layer11_rebuild(
    void *context,
    char *error,
    size_t error_bytes);
...
/* Checksums the complete first-chunk layer-2 input HC, layer-3 ingress,
 * Q/KV/compressor, attention-HC, and FFN-HC boundaries in that order. */
