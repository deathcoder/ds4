...
fn run_prefill_layers012_attention_loop_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("native complete layers 0-12 at 2K: 2048 raw rows + 512 dense layer-2/layer-4/layer-6/layer-8/layer-10/layer-12 compressed rows + 16 layer-3/layer-5/layer-7/layer-9/layer-11 compressed rows");
    println!(
    ...
    println!("scope: complete native layers 0-12, including exact paired ratio-4 and ratio-128 compressors, dense mixed attention, biased top-6 routed/shared FFNs, and additive final HC updates at the 2K prompt boundary; no layer-13 prefill, sparse post-prompt top-k, complete-model-prefill, output-logit, or throughput claim");
    if let Some(path) = json_path {
