...
use rust_star_runtime::metal::{
    ...
    Ratio128CompressorReplayProbeReport, RetainedSparseBoundaryProbeReport, RopeKvStoreProbeReport,
...
fn run_prefill_layers012_attention_loop_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("native complete layers 0-6 at 2K: 2048 raw rows + 512 dense layer-2/layer-4/layer-6 compressed rows + 16 layer-3 and 16 layer-5 compressed rows");
    println!(
    ...
    println!("scope: complete native layers 0/1/2/3/4/5/6, including exact layer-6 paired ratio-4 compressors, dense mixed attention, biased top-6 routed/shared FFN, and additive final HC update at the 2K prompt boundary; no layer-7 prefill, sparse post-prompt top-k, complete-model-prefill, or throughput claim");
    if let Some(path) = json_path {
...
fn prefill_layers012_attention_loop_probe_usage() -> &'static str {
    "usage: rust-star prefill-layers012-attention-loop-probe MODEL.gguf [--json PATH]\n\nRuns all 64 native 32-row schedules over positions 0--2047 in one persistent Metal context and completes layers 2 through 8. Layers 6 and 8 include paired ratio-4 attention/indexer compressors; layer 7 uses its ratio-128 compressor. Each layer continues through dense mixed attention, biased top-6 routed/shared experts, and its additive final HC update. Every retained boundary must match repeated DwarfStar captures bit-for-bit. The 2,064 logical layer-3/layer-5/layer-7 keys use a 2,112-row masked physical extent required by the 64-row FlashAttention block contract. Exactly 512 ratio-4 rows remain on the dense path at this prompt boundary. This does not claim sparse post-prompt ratio-4 attention, complete-model prefill, output logits, or throughput."
}
