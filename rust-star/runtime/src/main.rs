...
fn run_prefill_layers012_attention_loop_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("native layer-2 exact-2K completion plus layer-3 raw-KV ownership: 2048 raw rows + 512 dense layer-2 compressed rows");
    println!(
    ...
    println!("scope: complete native layers 0/1/2 at the exact 2K prompt boundary, followed without a host HC handoff by exact layer-3 HC ingress, Q/KV setup, compressed RoPE, FP8 finalization, and 2,048 retained raw-KV rows; no layer-3 compressor/attention/FFN, sparse post-prompt top-k, complete-model-prefill, or throughput claim");
    if let Some(path) = json_path {
...
fn prefill_layers012_attention_loop_probe_usage() -> &'static str {
    "usage: rust-star prefill-layers012-attention-loop-probe MODEL.gguf [--json PATH]\n\nRuns all 64 native 32-row schedules over positions 0--2047 in one persistent Metal context, then completes layer 2 in one terminal batch and continues directly through layer-3 HC ingress, Q/KV setup, compressed RoPE, FP8 finalization, and retained raw-KV ownership. Full layer-2 attention/HC identities and exact layer-3 final-tile boundaries must match repeated DwarfStar captures. Exactly 512 layer-2 compressed rows remain dense; sparse indexer top-k begins only after this prompt boundary. This does not claim layer-3 compression/attention/FFN, complete-model prefill, sparse ratio-4 decode, or throughput."
}
