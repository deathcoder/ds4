...
fn run_prefill_layers012_attention_loop_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("native complete layers 0-4 plus layer-5 Q/KV at 2K: 2048 raw rows + 512 dense layer-2/layer-4 compressed rows + 16 layer-3 compressed rows");
    println!(
    ...
    println!("scope: complete native layers 0/1/2/3/4 plus exact layer-5 HC attention ingress and finalized Q/KV state at the 2K prompt boundary; no layer-5 compressor, attention, FFN, sparse post-prompt top-k, complete-model-prefill, or throughput claim");
    if let Some(path) = json_path {
...
fn prefill_layers012_attention_loop_probe_usage() -> &'static str {
    "usage: rust-star prefill-layers012-attention-loop-probe MODEL.gguf [--json PATH]\n\nRuns all 64 native 32-row schedules over positions 0--2047 in one persistent Metal context, completes layers 2 through 4, then carries layer 4's final HC directly through layer-5 HC attention ingress and finalized Q/KV state. Every retained layer-2/layer-3/layer-4 boundary and all ten layer-5 final-tile boundaries must match repeated DwarfStar captures bit-for-bit. The 2,064 logical layer-3 keys use a 2,112-row masked physical extent required by the 64-row FlashAttention block contract. This does not claim the layer-5 compressor, attention, FFN, sparse ratio-4 decode, complete-model prefill, or throughput."
}
