...
fn run_prefill_layers012_attention_loop_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("scope: complete native layers 0-28, including exact paired ratio-4 and ratio-128 compressors, dense mixed attention, biased top-6 routed/shared FFNs, and additive final HC updates at the 2K prompt boundary; no layer-29 prefill, sparse post-prompt top-k, complete-model-prefill, output-logit, or throughput claim");
    if let Some(path) = json_path {
