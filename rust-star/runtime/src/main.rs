...
use rust_star_runtime::metal::{
    ...
    write_retained_engine_run_json, write_retained_sparse_boundary_probe_json,
    ...
    AttentionOutputNsgBenchReport, AttentionOutputProbeReport, AttentionReadProbeReport,
    AttentionRopeFusionBenchReport, AttentionSetupProbeReport, ClosedLoopDecoderProbeReport,
...
fn run() -> Result<()> {
    ...
    if command == "attention-rope-fusion-bench" {
        return run_attention_rope_fusion_bench_command(arguments.collect());
    }
    ...
    if command == "routed-nsg-bench" {
        return run_routed_nsg_bench_command(arguments.collect());
    }
    ...
    if command == "engine-retained-measure" {
        return run_retained_engine_measurement_command(arguments.collect());
    }
...
fn run_cold_prefill_decoder_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("paired protocol: ineligible one-token prefill control; exact native 2K-to-decode sparse correctness is covered by the dedicated frontier probe");
    if let Some(path) = json_path {
...
fn run_prefill_frontier_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("paired protocol: ineligible sequential-replay control; exact native batched prefill-to-sparse-decode correctness is covered by the dedicated frontier probe");
    if let Some(path) = json_path {
...
fn run_prefill_decode_frontier_probe_command(arguments: Vec<OsString>) -> Result<()> {
    if arguments.is_empty() {
        return Err(Error::invalid(prefill_decode_frontier_probe_usage()));
    }
    if matches!(arguments[0].to_str(), Some("--help") | Some("-h")) {
        println!("{}", prefill_decode_frontier_probe_usage());
        return Ok(());
    }
    let model_path = PathBuf::from(&arguments[0]);
    let mut json_path: Option<PathBuf> = None;
    ...
    while let Some(argument) = arguments.next() {
        match argument.to_str() {
            ...
            _ => return Err(Error::invalid(prefill_decode_frontier_probe_usage())),
        }
    }
    let model = MappedModel::open(&model_path)?;
    validate_resident_q2(model.gguf())?;
    let report = run_prefill_decode_frontier_probe(&model)?;
    println!(
        "exact native prefill: {} tokens, {} dispatches, token {}",
        report.prefill_tokens, report.prefill_dispatches, report.first_input_token,
    );
    println!(
        "GPU state handoff: {} layers, {} blit copies, wall={:.3} ms gpu={:.3} ms",
    ...
    println!(
        "closed-loop decode: positions {}-{}, {} exact selections, {:.3} positions/s",
        report.position_start,
        report.position_end,
        report.evaluated_positions,
        f64::from(report.evaluated_positions) * 1000.0 / report.decode_wall_ms,
    );
    println!(
        "production sparse boundary: position {}, 1025 ratio-4 rows, token {} -> {}, full logits C0 exact",
        report.position_end, report.final_input_token, report.final_selected_token,
    );
    println!(
        "scope: exact correctness control with per-position synchronization; no throughput claim"
    );
    if let Some(path) = json_path {
...
fn parse_engine_measurement_arguments(
    arguments: Vec<OsString>,
    usage: &'static str,
) -> Result<Option<(PathBuf, u32, u32, PathBuf)>> {
    if arguments.is_empty() {
    ...
    Ok(Some((model_path, context_tokens, gen_tokens, json_path)))
}
...
fn run_engine_measurement_command(arguments: Vec<OsString>, profile_layers: bool) -> Result<()> {
    ...
    let Some((model_path, context_tokens, gen_tokens, json_path)) =
        parse_engine_measurement_arguments(arguments, usage)?
    else {
        println!("{usage}");
        return Ok(());
    };
...
fn run_retained_engine_measurement_command(arguments: Vec<OsString>) -> Result<()> {
    let usage = retained_engine_measurement_usage();
    let Some((model_path, context_tokens, gen_tokens, json_path)) =
        parse_engine_measurement_arguments(arguments, usage)?
    else {
        println!("{usage}");
    ...
    let model = MappedModel::open(&model_path)?;
    validate_resident_q2(model.gguf())?;
    let report = run_retained_engine_measurement(&model, context_tokens, gen_tokens)?;
    write_retained_engine_run_file(&json_path, &report)?;
    let first_prompt_ms = report.first_load.model_warm_ms + report.first_load.prefill_ms;
    println!(
        "Rust Star retained lifecycle: first-load prompt {:.3} ms, retained prompt {:.3} ms ({:.3}x)",
        first_prompt_ms,
        report.retained.prefill_ms,
        first_prompt_ms / report.retained.prefill_ms,
    );
    println!(
        "retained prefill: {:.3} tok/s; generation: {:.3} tok/s; both 128-token transcripts exact",
        f64::from(report.retained.context) * 1000.0 / report.retained.prefill_ms,
        f64::from(report.retained.gen_tokens) * 1000.0 / report.retained.gen_ms,
    );
    println!("paired protocol: ineligible same-process lifecycle diagnostic");
    println!("json: {}", json_path.display());
    Ok(())
}
...
fn run_routed_nsg_bench_command(arguments: Vec<OsString>) -> Result<()> {
    if arguments.is_empty() {
        return Err(Error::invalid(routed_nsg_bench_usage()));
    }
    if matches!(arguments[0].to_str(), Some("--help") | Some("-h")) {
        println!("{}", routed_nsg_bench_usage());
        return Ok(());
    }
    let model_path = PathBuf::from(&arguments[0]);
    ...
    while let Some(argument) = arguments.next() {
        match argument.to_str() {
            ...
            _ => return Err(Error::invalid(routed_nsg_bench_usage())),
        }
    }
    let model = MappedModel::open(&model_path)?;
    validate_resident_q2(model.gguf())?;
    let report = run_routed_nsg_bench(&model, warmup_rounds, measured_rounds)?;
    let gpu_change = (report.nsg4_gpu.median_ms / report.nsg2_gpu.median_ms - 1.0) * 100.0;
    println!("fixture: {}", report.fixture_id);
    println!(
        "execution: {} warmup + {} measured alternating rounds per NSG",
        report.warmup_rounds, report.measured_rounds
    );
    println!(
        "NSG=2: wall median={:.3} ms MAD={:.3} ms; gpu median={:.3} ms MAD={:.3} ms",
    ...
    println!(
        "NSG=4: wall median={:.3} ms MAD={:.3} ms; gpu median={:.3} ms MAD={:.3} ms ({gpu_change:+.2}%)",
        report.nsg4_wall.median_ms,
        report.nsg4_wall.mad_ms,
        report.nsg4_gpu.median_ms,
        report.nsg4_gpu.mad_ms,
    );
    println!("result: every routed intermediate and output matched DwarfStar bit-for-bit");
    if let Some(path) = json_path {
        write_routed_nsg_bench_file(&path, &report)?;
        println!("json: {}", path.display());
    }
    Ok(())
}
...
fn run_attention_rope_fusion_bench_command(arguments: Vec<OsString>) -> Result<()> {
    if arguments.is_empty() {
        return Err(Error::invalid(attention_rope_fusion_bench_usage()));
    }
    if matches!(arguments[0].to_str(), Some("--help") | Some("-h")) {
        println!("{}", attention_rope_fusion_bench_usage());
        return Ok(());
    }
    let model_path = PathBuf::from(&arguments[0]);
    let mut warmup_rounds = 10_u32;
    ...
    while let Some(argument) = arguments.next() {
        match argument.to_str() {
            ...
            _ => return Err(Error::invalid(attention_rope_fusion_bench_usage())),
        }
    }
    let model = MappedModel::open(&model_path)?;
    validate_resident_q2(model.gguf())?;
    let report = run_attention_rope_fusion_bench(&model, warmup_rounds, measured_rounds)?;
    ...
    println!(
        "separate (18 dispatches): wall median={:.3} ms MAD={:.3} ms; gpu median={:.3} ms MAD={:.3} ms",
    ...
    println!(
        "fused (17 dispatches): wall median={:.3} ms MAD={:.3} ms; gpu median={:.3} ms MAD={:.3} ms ({gpu_change:+.2}%)",
    ...
    println!(
        "result: both paths preserved the pre-RoPE boundary and matched every pinned attention-output boundary bit-for-bit"
    );
    if let Some(path) = json_path {
        write_attention_rope_fusion_bench_file(&path, &report)?;
        println!("json: {}", path.display());
    }
    Ok(())
}
...
fn run_qb_rows_bench_command(arguments: Vec<OsString>) -> Result<()> {
    if arguments.is_empty() {
        return Err(Error::invalid(qb_rows_bench_usage()));
    }
    if matches!(arguments[0].to_str(), Some("--help") | Some("-h")) {
        println!("{}", qb_rows_bench_usage());
        return Ok(());
    }
    let model_path = PathBuf::from(&arguments[0]);
    let mut candidate_rows = 4_u32;
    let mut warmup_rounds = 10_u32;
    let mut measured_rounds = 50_u32;
    let mut json_path: Option<PathBuf> = None;
    let mut arguments = arguments.into_iter().skip(1);
    while let Some(argument) = arguments.next() {
        match argument.to_str() {
            Some("--rows") => {
                candidate_rows = parse_u32_option("--rows", arguments.next().as_deref())?;
            }
            Some("--warmup") => {
                warmup_rounds = parse_u32_option("--warmup", arguments.next().as_deref())?;
            }
            ...
            _ => return Err(Error::invalid(qb_rows_bench_usage())),
        }
    }
    let model = MappedModel::open(&model_path)?;
    validate_resident_q2(model.gguf())?;
    let report = run_qb_rows_bench(&model, candidate_rows, warmup_rounds, measured_rounds)?;
    let gpu_change = (report.candidate_gpu.median_ms / report.baseline_gpu.median_ms - 1.0) * 100.0;
    println!("fixture: {}", report.fixture_id);
    println!(
        "execution: {} warmup + {} measured alternating rounds per path",
    ...
    println!(
        "Q-B rows=2: wall median={:.3} ms MAD={:.3} ms; gpu median={:.3} ms MAD={:.3} ms",
        report.baseline_wall.median_ms,
        report.baseline_wall.mad_ms,
        report.baseline_gpu.median_ms,
        report.baseline_gpu.mad_ms,
    ...
    println!(
        "Q-B rows={}: wall median={:.3} ms MAD={:.3} ms; gpu median={:.3} ms MAD={:.3} ms ({gpu_change:+.2}%)",
...
fn run_long_prefill_bootstrap_probe_command(arguments: Vec<OsString>) -> Result<()> {
    if arguments.is_empty() {
        return Err(Error::invalid(long_prefill_bootstrap_probe_usage()));
    }
    if matches!(arguments[0].to_str(), Some("--help") | Some("-h")) {
        println!("{}", long_prefill_bootstrap_probe_usage());
        return Ok(());
    }
    let model_path = PathBuf::from(&arguments[0]);
    let mut json_path: Option<PathBuf> = None;
    ...
    while let Some(argument) = arguments.next() {
        match argument.to_str() {
            ...
            _ => return Err(Error::invalid(long_prefill_bootstrap_probe_usage())),
        }
    }
    let model = MappedModel::open(&model_path)?;
    validate_resident_q2(model.gguf())?;
    let report = run_long_prefill_bootstrap_probe(&model)?;
    println!(
        "native long-prefill bootstrap: accepted 32K token stream, first 4096-token chunk in {} x {}-row tiles",
        report.tiles, report.tile_rows
    );
    println!("retained state: 4096 raw rows for layers 0/1/2, 1024 layer-2 attention/indexer compressed rows, recurrent compressor tails retained");
    println!(
        "mapping: {}/{} no-copy model ranges; {} dispatches; summed wall={:.3} ms gpu={:.3} ms",
    ...
    println!("scope: first native 4K chunk and retained-state lifetime only; no intermediate C0, complete-32K, or throughput claim");
    if let Some(path) = json_path {
        write_long_prefill_bootstrap_probe_file(&path, &report)?;
        println!("json: {}", path.display());
    }
    Ok(())
}
...
fn run_long_prefill_continuation_bootstrap_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!(
        "layer-2 evidence: position 4099 isolated sparse control C0; positions {}..{} complete native layer tile C0 across 3 sparse and {} downstream outputs, 10+{} dispatches, and {}/{} no-copy mappings",
        report.layer2_tail_start,
        report.layer2_tail_start + report.layer2_tail_rows - 1,
    ...
    println!(
        "layer-3 evidence: live layer-2 final HC and native retained pre-4096 attention history feed positions 4096..4127 through the complete native tile ({} ingress/QKV dispatches, {} total dispatches, {} exact outputs, {}/{} no-copy mappings)",
    ...
    println!(
        "layer-2/layer-3 loop: positions 4096..8191 executed in {} retained 32-row tiles ({} exact C0 anchors, {} structurally executed tiles, {} ratio-128 emissions, {} dispatches, {}/{} no-copy mappings; final HC/state checksums={}/{}/{}; full layer-2 checksum={}; tiled layer-3 checksum={}; {}/128 layer-3 tile checksums match production geometry)",
    ...
    println!(
        "layer-3 production batch: aligned ratio-128 compressor, attention, and FFN replay use {} dispatches and {}/{} no-copy mappings; full checksum={} with {}/128 production tile matches",
    ...
    println!(
        "layer-5 complete prefix: exact 4096-row ingress, QKV, ratio-128 compression, padded Flash attention, and FFN use {} dispatches and {}/{} no-copy mappings; all 6 boundary/tail checksums match the 4K oracle",
    ...
    println!(
        "layer-7 complete prefix: exact 4096-row ingress, QKV, ratio-128 compression, padded Flash attention, and FFN use {} dispatches and {}/{} no-copy mappings; all 8 boundary/tail hashes match the 4K oracle",
        report.layer7_prefix_complete_dispatches,
        report.layer7_prefix_complete_pointer_matches,
        report.layer7_prefix_complete_wrapped_model_ranges,
    );
    println!(
        "layer-7 complete continuation: {} exact 32-row ingress/QKV tiles plus the production compressor, Flash attention, and FFN use {} dispatches and {}/{} no-copy mappings; all 4 ingress hashes, compressed hash={}, and 3 attention/FFN tail hashes match the 8K oracle",
        report.layer7_continuation_tiles,
        report.layer7_continuation_dispatches,
        report.layer7_continuation_pointer_matches,
        report.layer7_continuation_wrapped_model_ranges,
    ...
    println!(
        "layer-9 complete prefix: exact 4096-row ingress, QKV, ratio-128 compression, padded Flash attention, and FFN use {} dispatches and {}/{} no-copy mappings; all 8 boundary/tail hashes match the 4K oracle",
    ...
    println!(
        "layer-9 complete continuation: {} exact 32-row ingress/QKV tiles plus the production compressor, Flash attention, and FFN use {} dispatches and {}/{} no-copy mappings; all 4 ingress hashes, compressed hash={}, and 3 attention/FFN tail hashes match the 8K oracle",
    ...
    println!(
        "layer-10 exact boundary: prefix and continuation ingress, QKV, both ratio-4 compressed caches, and all recurrent states match 20/20 oracle hashes over {}+{} dispatches and {}/{}+{}/{} no-copy mappings",
        report.layer10_prefix_boundary_dispatches,
        report.layer10_continuation_boundary_dispatches,
        report.layer10_prefix_boundary_pointer_matches,
        report.layer10_prefix_boundary_wrapped_model_ranges,
        report.layer10_continuation_boundary_pointer_matches,
        report.layer10_continuation_boundary_wrapped_model_ranges,
    );
    println!(
        "layer-10 complete prefix and continuation: prefix sparse attention/FFN uses {} dispatches and {}/{} no-copy mappings; {} continuation tiles use {} dispatches and {}/{} mappings; all 6 KQV-back/post-attention/post-FFN hashes match the 8K oracle",
        report.layer10_prefix_complete_dispatches,
        report.layer10_prefix_complete_pointer_matches,
        report.layer10_prefix_complete_wrapped_model_ranges,
        report.layer10_continuation_tiles,
        report.layer10_continuation_dispatches,
        report.layer10_continuation_pointer_matches,
        report.layer10_continuation_wrapped_model_ranges,
    );
    println!(
        "layer-11 complete prefix: exact 4096-row ingress, QKV, ratio-128 compression, padded Flash attention, and FFN use {} dispatches and {}/{} no-copy mappings; all 8 boundary/tail hashes match the 4K oracle",
    ...
    println!(
        "layer-11 complete continuation: {} exact 32-row ingress/QKV tiles plus the production compressor, Flash attention, and FFN use {} dispatches and {}/{} no-copy mappings; all 4 ingress hashes, compressed hash={}, and 3 attention/FFN tail hashes match the 8K oracle",
    ...
    println!(
        "layer-12 exact boundary: prefix and continuation ingress, QKV, both ratio-4 compressed caches, and all recurrent states match 20/20 oracle hashes over {}+{} dispatches and {}/{}+{}/{} no-copy mappings",
        report.layer12_prefix_boundary_dispatches,
        report.layer12_continuation_boundary_dispatches,
        report.layer12_prefix_boundary_pointer_matches,
        report.layer12_prefix_boundary_wrapped_model_ranges,
        report.layer12_continuation_boundary_pointer_matches,
        report.layer12_continuation_boundary_wrapped_model_ranges,
    );
    println!(
        "layer-12 complete prefix and continuation: prefix sparse attention/FFN uses {} dispatches and {}/{} no-copy mappings; {} continuation tiles use {} dispatches and {}/{} mappings; all 6 KQV-back/post-attention/post-FFN hashes match the 8K oracle",
        report.layer12_prefix_complete_dispatches,
        report.layer12_prefix_complete_pointer_matches,
        report.layer12_prefix_complete_wrapped_model_ranges,
        report.layer12_continuation_tiles,
        report.layer12_continuation_dispatches,
        report.layer12_continuation_pointer_matches,
        report.layer12_continuation_wrapped_model_ranges,
    );
    println!(
        "scope: complete retained layers 2 through 12 match the second 4K chunk oracle. Layers 13..42, complete 8K transformer output, output-logit C0, and throughput remain unclaimed"
    );
    if let Some(path) = json_path {
...
fn run_long_prefill_transformer_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    if matches!(arguments[0].to_str(), Some("--help") | Some("-h")) {
        println!("{}", long_prefill_transformer_probe_usage());
        return Ok(());
    }
    let model_path = PathBuf::from(&arguments[0]);
    let mut json_path: Option<PathBuf> = None;
    ...
    while let Some(argument) = arguments.next() {
        match argument.to_str() {
            ...
            _ => return Err(Error::invalid(long_prefill_transformer_probe_usage())),
        }
    }
    let model = MappedModel::open(&model_path)?;
    validate_resident_q2(model.gguf())?;
    let report = run_long_prefill_transformer_probe(&model)?;
    println!(
        "native long-prefill transformer: accepted 32K token stream, first 4096-token chunk through layers 0-42"
    );
    println!(
        "retained state: 4096 raw rows per layer, 1024 ratio-4 and 32 ratio-128 compressed rows per applicable layer"
    );
    println!(
        "schedule: bootstrap {} dispatches and transformer {} dispatches; {}/{} transformer no-copy ranges; selected token {}",
    ...
    println!(
        "timing: bootstrap wall={:.3} ms gpu={:.3} ms; transformer wall={:.3} ms gpu={:.3} ms; output head wall={:.3} ms gpu={:.3} ms",
    ...
    println!("scope: complete native 4K transformer schedule and retained-state lifetime; no intermediate C0, selected-token oracle match, complete-32K, or throughput claim");
    if let Some(path) = json_path {
        write_long_prefill_transformer_probe_file(&path, &report)?;
        println!("json: {}", path.display());
    }
    Ok(())
}
...
fn run_prefill_layers012_attention_loop_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("native complete layers 0-42 at 2K: 2048 raw rows + 512 dense layer-2/layer-4/layer-6/layer-8/layer-10/layer-12/layer-14/layer-16/layer-18/layer-20/layer-22/layer-24/layer-26/layer-28/layer-30/layer-32/layer-34/layer-36/layer-38/layer-40/layer-42 compressed rows + 16 layer-3/layer-5/layer-7/layer-9/layer-11/layer-13/layer-15/layer-17/layer-19/layer-21/layer-23/layer-25/layer-27/layer-29/layer-31/layer-33/layer-35/layer-37/layer-39/layer-41 compressed rows");
    println!(
...
fn write_long_prefill_bootstrap_probe_file(
    path: &Path,
    report: &LongPrefillBootstrapProbeReport,
) -> Result<()> {
    let temporary = path.with_extension(format!(
        "{}tmp",
        path.extension()
    ...
    let file = File::create(&temporary).map_err(|error| {
        Error::invalid(format!(
            "cannot create long-prefill bootstrap JSON {}: {error}",
            temporary.display()
        ))
    })?;
    let mut output = BufWriter::new(file);
    write_long_prefill_bootstrap_probe_json(&mut output, report)?;
    output.flush()?;
    drop(output);
    std::fs::rename(&temporary, path).map_err(|error| {
        Error::invalid(format!(
...
fn write_long_prefill_transformer_probe_file(
    path: &Path,
    ...
    let file = File::create(&temporary).map_err(|error| {
        Error::invalid(format!(
            "cannot create long-prefill transformer JSON {}: {error}",
            temporary.display()
        ))
    })?;
    let mut output = BufWriter::new(file);
    write_long_prefill_transformer_probe_json(&mut output, report)?;
    output.flush()?;
    drop(output);
    std::fs::rename(&temporary, path).map_err(|error| {
        Error::invalid(format!(
            "cannot install long-prefill transformer JSON {}: {error}",
            path.display()
        ))
    })?;
    Ok(())
...
fn write_attention_rope_fusion_bench_file(
    path: &Path,
    ...
    let file = File::create(&temporary).map_err(|error| {
        Error::invalid(format!(
            "cannot create attention RoPE-fusion benchmark JSON {}: {error}",
            temporary.display()
        ))
    })?;
    let mut output = BufWriter::new(file);
    write_attention_rope_fusion_bench_json(&mut output, report)?;
    output.flush()?;
    drop(output);
    std::fs::rename(&temporary, path).map_err(|error| {
        Error::invalid(format!(
            "cannot install attention RoPE-fusion benchmark JSON {}: {error}",
            path.display()
        ))
    })?;
    Ok(())
...
fn write_prefill_decode_frontier_probe_file(
    path: &Path,
    ...
    let file = File::create(&temporary).map_err(|error| {
        Error::invalid(format!(
            "cannot create prefill-to-decode frontier JSON {}: {error}",
            temporary.display()
        ))
    })?;
    let mut output = BufWriter::new(file);
    write_prefill_decode_frontier_probe_json(&mut output, report)?;
    output.flush()?;
    drop(output);
    std::fs::rename(&temporary, path).map_err(|error| {
        Error::invalid(format!(
            "cannot install prefill-to-decode frontier JSON {}: {error}",
            path.display()
        ))
    })?;
    Ok(())
...
fn write_retained_engine_run_file(path: &Path, report: &RetainedEngineRunReport) -> Result<()> {
    let temporary = path.with_extension(format!(
        "{}tmp",
        path.extension()
            .and_then(OsStr::to_str)
    ...
    let file = File::create(&temporary).map_err(|error| {
        Error::invalid(format!(
            "cannot create Rust Star retained engine-run JSON {}: {error}",
            temporary.display()
        ))
    })?;
    let mut output = BufWriter::new(file);
    write_retained_engine_run_json(&mut output, report)?;
    output.flush()?;
    drop(output);
    std::fs::rename(&temporary, path).map_err(|error| {
        Error::invalid(format!(
            "cannot install Rust Star retained engine-run JSON {}: {error}",
            path.display()
        ))
    })?;
    Ok(())
...
fn write_routed_nsg_bench_file(path: &Path, report: &RoutedNsgBenchReport) -> Result<()> {
    let temporary = path.with_extension(format!(
        "{}tmp",
        path.extension()
            .and_then(OsStr::to_str)
    ...
    let file = File::create(&temporary).map_err(|error| {
        Error::invalid(format!(
            "cannot create routed NSG benchmark JSON {}: {error}",
            temporary.display()
        ))
    })?;
    let mut output = BufWriter::new(file);
    write_routed_nsg_bench_json(&mut output, report)?;
    output.flush()?;
    drop(output);
    std::fs::rename(&temporary, path).map_err(|error| {
        Error::invalid(format!(
            "cannot install routed NSG benchmark JSON {}: {error}",
            path.display()
        ))
    })?;
    Ok(())
...
fn write_qb_rows_bench_file(path: &Path, report: &QbRowsBenchReport) -> Result<()> {
    let temporary = path.with_extension(format!(
        "{}tmp",
        path.extension()
            .and_then(OsStr::to_str)
    ...
    let file = File::create(&temporary).map_err(|error| {
        Error::invalid(format!(
            "cannot create Q-B row benchmark JSON {}: {error}",
            temporary.display()
        ))
    })?;
    let mut output = BufWriter::new(file);
    write_qb_rows_bench_json(&mut output, report)?;
    output.flush()?;
    drop(output);
    std::fs::rename(&temporary, path).map_err(|error| {
        Error::invalid(format!(
            "cannot install Q-B row benchmark JSON {}: {error}",
            path.display()
        ))
    })?;
    Ok(())
...
fn long_prefill_continuation_bootstrap_probe_usage() -> &'static str {
    "usage: rust-star long-prefill-continuation-bootstrap-probe MODEL.gguf [--json PATH]\n\nRuns the first 4,096-token chunk through the complete native transformer, then advances positions 4,096--8,191 in the same retained Metal context. It verifies the layer-2 sparse transition and complete retained layers 2 through 12 against the repeated production oracle. This diagnostic does not claim layers 13--42, the complete 8K transformer output, output-logit C0, throughput, or a speedup."
}
...
fn long_prefill_transformer_probe_usage() -> &'static str {
    "usage: rust-star long-prefill-transformer-probe MODEL.gguf [--json PATH]\n\nRuns the first 4,096-token native chunk from the accepted oracle-v3 32K token stream through complete layers 0 through 42 and the output head. It verifies the expanded retained-state schedule and no-copy ownership, but makes no intermediate C0, selected-token oracle-match, complete-32K, or throughput claim."
}
...
fn attention_rope_fusion_bench_usage() -> &'static str {
    "usage: rust-star attention-rope-fusion-bench MODEL.gguf [--warmup N] [--iterations N] [--json PATH]\n\nAlternates separate FlashAttention reduction plus inverse RoPE against the fused 17-dispatch chain in one Metal context, preserves the pre-RoPE diagnostic boundary, checks every downstream output bit-for-bit, and reports diagnostic timing only."
}
...
fn routed_nsg_bench_usage() -> &'static str {
    "usage: rust-star routed-nsg-bench MODEL.gguf [--warmup N] [--iterations N] [--json PATH]\n\nAlternates NSG=2 and NSG=4 in one Metal context on the pinned layer-0 MoE input, checks both outputs bit-for-bit, and reports diagnostic timing only."
}

fn layer0_probe_usage() -> &'static str {
    "usage: rust-star layer0-probe MODEL.gguf [--json PATH]\n\nRuns the complete 28-dispatch layer-0 decode path in one Metal command buffer and checks every retained attention, router, expert, and HC-state boundary."
}
...
fn retained_engine_measurement_usage() -> &'static str {
    "usage: rust-star engine-retained-measure MODEL.gguf --context 2048 --gen-tokens 128 --json PATH\n\nRuns two exact 2K/128 engine cycles in one process and one Metal context. The first cycle fully charges model-page warming, context creation, and model residency. The second cycle reuses the established model mapping, Metal context, view cache, and residency set. Both transcripts must match the oracle. This lifecycle diagnostic is deliberately ineligible for the fresh-process paired protocol."
}
