...
fn run_long_prefill_continuation_bootstrap_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!(
        "scope: complete retained layer-2 and production-batch layer-3 outputs match the 8K oracle; layer 4 is the next continuation boundary, while layers 4..42, complete 8K transformer output, output-logit C0, and throughput remain unclaimed"
    );
    if let Some(path) = json_path {
