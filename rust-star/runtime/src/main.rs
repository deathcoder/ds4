...
fn run_prefill_layers012_attention_loop_probe_command(arguments: Vec<OsString>) -> Result<()> {
    ...
    println!("native layer-2 exact-2K completion plus complete layer-3 prefill: 2048 raw rows + 512 dense layer-2 compressed rows + 16 layer-3 compressed rows");
    println!(
    ...
    println!("scope: complete native layers 0/1/2/3 at the exact 2K prompt boundary, including layer-3 ratio-128 compression, dense mixed attention, biased top-k MoE routing, routed/shared experts, and additive FFN HC post; no layer-4 prefill, sparse post-prompt top-k, complete-model-prefill, or throughput claim");
    if let Some(path) = json_path {
...
fn prefill_layers012_attention_loop_probe_usage() -> &'static str {
    "usage: rust-star prefill-layers012-attention-loop-probe MODEL.gguf [--json PATH]\n\nRuns all 64 native 32-row schedules over positions 0--2047 in one persistent Metal context, then completes layer 2 in one terminal batch and continues directly through complete layer-3 prefill: HC ingress, Q/KV setup, compressed RoPE, FP8 finalization, the full ratio-128 attention compressor, dense mixed attention, output projections, biased top-k MoE routing, routed/shared experts, and additive FFN HC post. Every retained layer-2/layer-3 boundary must match repeated DwarfStar captures. The 2,064 logical layer-3 keys use a 2,112-row masked physical extent required by the 64-row FlashAttention block contract. This does not claim layer-4 prefill, complete-model prefill, sparse ratio-4 decode, or throughput."
}
