    ...
    size_t error_bytes)
{
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> model_buffers[136] = { nil };
        NSUInteger inner[136] = { 0 };
        BOOL matches[136] = { NO };
        for (uint32_t index = 0; index < 136u; index++) {
            model_buffers[index] = wrap_model_range(
                context, model_mapping, model_bytes, offsets[index], sizes[index],
                &inner[index], &matches[index], error, error_bytes);
            if (!model_buffers[index]) return 0;
...
#undef RUST_STAR_ENCODE_L6_SHARED
        ...
        result->dispatches = 266u;
        result->wrapped_model_ranges = 136u;
        result->pointer_matches = pointer_matches;
