    ...
    size_t error_bytes)
{
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> model_buffers[164] = { nil };
        NSUInteger inner[164] = { 0 };
        BOOL matches[164] = { NO };
        for (uint32_t index = 0; index < 164u; index++) {
            model_buffers[index] = wrap_model_range(
                context, model_mapping, model_bytes, offsets[index], sizes[index],
                &inner[index], &matches[index], error, error_bytes);
            if (!model_buffers[index]) return 0;
...
#undef RUST_STAR_ENCODE_L7_SHARED
        ...
        result->dispatches = 313u;
        result->wrapped_model_ranges = 164u;
        result->pointer_matches = pointer_matches;
