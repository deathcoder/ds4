...
To complete native 2K prefill through layer 40:
...
attention/indexer compressors, dense mixed attention, routed/shared FFN, and
additive final HC update. Every retained layer-7 through layer-40 boundary, full
attention output, compressor state, and full HC identity matches fresh
DwarfStar processes exactly. The combined terminal schedule uses 2,255
dispatches and 1,156/1,156 no-copy model views.

Exactly 512 compressed rows for each even layer from 2 through 40 still use the
dense path; each odd compressed layer through layer 23 retains 16 ratio-128
rows, as do layers 25, 27, 29, 31, 33, 35, 37, and 39. The pinned default
remains dense through 1,024 rows; sparse top-k first
applies at 1,025 rows. Layer-41 prefill, sparse post-prompt integration,
