...
`rust-star-long-prefill-continuation-bootstrap-probe-v27`. The same retained
...
dispatches and 45/45 no-copy mappings. It then keeps the same allocations live
...
reported mismatch positions identify the tiled diagnostic's batch-geometry
limitation. Version 12 retains the full second-chunk norm/Q/KV tensors and
replays DwarfStar's aligned 4,096-row ratio-128 compressor pool before the
production attention and FFN tail. That 38-dispatch replay preserves 19/19
no-copy mappings, matches all 128 production tile checksums, and reaches the
exact full checksum `17010162403439886297`. The runtime then rebuilds layer 4's
first-4K prefix and refreshes ratio-4 state from the last four normalized tokens
...
Version 17 completes that first-4K layer-4 prefix instead of treating only its
ingress and compressors as evidence. On the M1 Ultra it follows DwarfStar's
legacy aligned-F16 route: tiled 4,096-by-1,024 indexer scores, the device-sized
top-512 selection, chronological sorting, F16 cache staging, heads8 indexed
...
2,176/2,176 no-copy mappings. Version 16 then consumes that exact layer-4 HC
through all 128 layer-5 continuation ingress/QKV tiles. The nine aggregate
...
layer-5 prefix is now complete and exact. Version 19 consumes the exact
second-chunk Q/KV/compressed history with DwarfStar's 4,288-key production
Flash geometry, then runs inverse RoPE, attention output, and the complete
FFN. The 38-dispatch production tail preserves 19/19 no-copy mappings and its
KQV-back, post-attention HC, and post-FFN HC hashes match two independent
captures. Version 20 binds that exact layer-5 HC into the proven even-layer
executor and rebuilds layer 6 through ingress, QKV, both complete ratio-4
compressed caches, and all recurrent compressor states. Prefix and continuation
each use 40 dispatches and 17/17 no-copy mappings; all ten hashes per half match
the repeated DwarfStar capture. Version 21 then runs the production prefix
sparse path and all 128 continuation sparse-attention/FFN tiles. The prefix
...
layers 2 through 7. Version 23 binds those exact prefix and continuation HCs
into layer 8's ratio-4 path. Both ingress/QKV/paired-compressor boundaries use
40 dispatches and 17/17 mappings and match all 20 hashes. The prefix
sparse-attention/FFN tail uses 36 dispatches and 17/17 mappings; its 128-tile
continuation uses 4,608 dispatches and 2,176/2,176 mappings. All six KQV-back,
post-attention HC, and post-FFN HC hashes match. The report therefore claims
complete retained layers 2 through 8. Version 24 preserves those exact prefix
and continuation HC buffers and executes layer 9 through ratio-128 ingress/QKV,
compression, padded Flash attention, and FFN. Its 49-dispatch prefix preserves
28/28 mappings; its 128 continuation tiles plus production tail use 1,318
dispatches and 1,171/1,171 mappings. All 16 boundary and tail hashes match the
repeated oracle. The report therefore claims complete retained layers 2 through
9. Version 25 binds those exact prefix and continuation HCs into layer 10's
ratio-4 path. Both ingress/QKV/paired-compressor boundaries use 40 dispatches
and 17/17 mappings and match all 20 hashes. The 36-dispatch prefix tail and all
128 continuation sparse-attention/FFN tiles match six more hashes over
2,176/2,176 continuation mappings. Version 26 preserves those exact prefix and
continuation HC buffers and executes layer 11 through ratio-128 ingress/QKV,
compression, padded Flash attention, and FFN. Its 49-dispatch prefix preserves
28/28 mappings; its 128 continuation tiles plus production tail use 1,318
dispatches and 1,171/1,171 mappings. All 16 boundary and tail hashes match the
repeated oracle. Version 27 binds the exact layer-11 prefix and continuation HCs
into layer 12's ratio-4 path. Both ingress/QKV/paired-compressor boundaries use
40 dispatches and 17/17 mappings and match all 20 hashes. The 36-dispatch
prefix tail and all 128 continuation sparse-attention/FFN tiles match six more
hashes over 2,176/2,176 continuation mappings. The report therefore claims
complete retained layers 2 through 12, but not layers 13--42, the complete 8K
