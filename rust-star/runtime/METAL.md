...
## Retained 4K-to-8K layer-2 through complete layer 12

Schema: `rust-star-long-prefill-continuation-bootstrap-probe-v27`.
...
4,096--8,191 through layers 0/1 and the paired layer-2 compressors. The M1
...
The exact gate continues the same live context through positions 4,128--4,159.
...
remain the only C0 anchors; the later 126 tiles are structural retained-state
...
boundary. Version 15 retains the normalized query/indexer inputs and executes
all 128 second-chunk layer-4 tiles with DwarfStar's sparse indexed top-512
attention. The existing grouped attention-output and biased routed/shared FFN
...
layer 6 is claimed. Version 22 retains its distinct prefix and continuation HC
buffers and feeds them through layer 7's proven odd-layer ratio-128 executor.
The exact prefix uses 49 dispatches and 28/28 no-copy mappings. The 128
continuation ingress/QKV tiles plus aligned compressor, padded Flash attention,
and FFN use 1,318 dispatches and 1,171/1,171 mappings. All 16 captured boundary
and tail hashes match, so complete retained layer 7 is claimed. Version 23
feeds its exact prefix and continuation HC into layer 8's even ratio-4 path.
Both paired-compressor boundaries match all 20 hashes over 40 dispatches and
17/17 no-copy mappings each. The prefix tail uses 36 dispatches and 17/17
mappings; all 128 continuation sparse-attention/FFN tiles use 4,608 dispatches
and 2,176/2,176 mappings. All six tail hashes match, so complete retained
layer 8 is claimed. Version 24 preserves those exact prefix and continuation HC
buffers and feeds them through layer 9's odd ratio-128 path. The exact prefix
uses 49 dispatches and 28/28 no-copy mappings. Its 128 continuation
ingress/QKV tiles, production compressor, padded Flash attention, and FFN use
1,318 dispatches and 1,171/1,171 mappings. All 16 boundary and tail hashes
match the repeated DwarfStar oracle, so complete retained layer 9 is claimed.
Version 25 feeds those exact prefix and continuation HCs into layer 10's even
ratio-4 path. Both paired-compressor boundaries match all 20 hashes over 40
dispatches and 17/17 no-copy mappings each. The prefix tail uses 36 dispatches
and 17/17 mappings; all 128 continuation sparse-attention/FFN tiles use 4,608
dispatches and 2,176/2,176 mappings. All six tail hashes match, so complete
retained layer 10 is claimed. Version 26 preserves those exact prefix and
continuation HC buffers and feeds them through layer 11's odd ratio-128 path.
The exact prefix uses 49 dispatches and 28/28 no-copy mappings. Its 128
continuation ingress/QKV tiles, production compressor, padded Flash attention,
and FFN use 1,318 dispatches and 1,171/1,171 mappings. All 16 boundary and tail
hashes match the repeated DwarfStar oracle, so complete retained layer 11 is
claimed. Version 27 feeds those exact HCs through layer 12's even ratio-4 path.
Both paired-compressor boundaries match all 20 hashes over 40 dispatches and
17/17 no-copy mappings each. The prefix tail uses 36 dispatches and 17/17
mappings; all 128 continuation sparse-attention/FFN tiles use 4,608 dispatches
and 2,176/2,176 mappings. All six tail hashes match, so complete retained layer
12 is claimed and layer 13 becomes the next boundary.
