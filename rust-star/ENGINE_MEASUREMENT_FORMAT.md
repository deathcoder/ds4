...
`prefill_representative_attention_stage_gpu_ms`, and
`prefill_layer_profile`. That mode
...
five attention-internal intervals for each representative layer, records the
...
Eligible `engine-measure` records require all three arrays and the profile to be
