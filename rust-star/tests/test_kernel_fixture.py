...
PREFILL_DECODE_FRONTIER_FIXTURE = (
    RUST_STAR_DIR / "fixtures" / "prefill-decode-frontier-4099-v1"
)
...
PREFILL_LAYER3_INGRESS_FIXTURE = (
    RUST_STAR_DIR / "fixtures" / "prefill-layer3-ingress-2048-v1"
)
...
class KernelFixtureTests(unittest.TestCase):
    ...
    def test_prefill_layer3_continuation_ingress_fixture_manifest_and_payloads(self) -> None:
        ...
        self.assertEqual(report["tensors"], 9)
        self.assertEqual(report["verified_bytes"], 5_767_168)
        self.assertEqual(manifest["scope"]["captured_position_range"], [4096, 4127])
        self.assertTrue(manifest["capture"]["fresh_process_bitwise_match"])
        self.assertTrue(manifest["claims"]["live_layer2_hc_input"])
        self.assertTrue(manifest["claims"]["complete_layer3_ingress_qkv_tile"])
        self.assertFalse(manifest["claims"]["complete_layer3_tile"])
    ...
    def test_prefill_layer2_continuation_tail_fixture_manifest_and_payloads(self) -> None:
        ...
        self.assertEqual(report["scope"], "layer-segment")
        self.assertEqual(report["operations"], 11)
        self.assertEqual(report["tensors"], 16)
        self.assertEqual(report["verified_bytes"], 14_026_240)
        self.assertEqual(manifest["scope"]["captured_position_range"], [4096, 4127])
        self.assertTrue(manifest["capture"]["fresh_process_bitwise_match"])
        self.assertTrue(manifest["claims"]["complete_downstream_tail_tile"])
        self.assertFalse(manifest["claims"]["oracle_seeded_kqv_back_input"])
        self.assertTrue(manifest["claims"]["native_multirow_sparse_kqv_back"])
        self.assertTrue(manifest["claims"]["complete_layer_tile"])
    ...
    def test_prefill_frontier_32768_fixture_manifest_and_payloads(self) -> None:
        manifest = json.loads(
            (PREFILL_FRONTIER_32768_FIXTURE / "manifest.json").read_text(
                encoding="utf-8"
            )
        )
        report = validate_differential_fixture(PREFILL_FRONTIER_32768_FIXTURE)
        self.assertEqual(
            report["fixture_id"],
        ...
        self.assertEqual(report["operations"], 1)
        self.assertEqual(report["tensors"], 2)
        self.assertEqual(report["verified_bytes"], 648_192)
        self.assertEqual(manifest["capture"]["prefill_tokens"], 32_768)
    ...
    def test_prefill_decode_frontier_fixture_manifest_and_payloads(self) -> None:
        manifest = json.loads(
            (PREFILL_DECODE_FRONTIER_FIXTURE / "manifest.json").read_text(
                encoding="utf-8"
            )
        )
        report = validate_differential_fixture(PREFILL_DECODE_FRONTIER_FIXTURE)
        self.assertEqual(
            report["fixture_id"],
        ...
        self.assertEqual(report["operations"], 2)
        self.assertEqual(report["tensors"], 3)
        self.assertEqual(report["verified_bytes"], 1_042_448)
        self.assertEqual(manifest["scope"]["prefill_tokens"], 2048)
        self.assertEqual(manifest["scope"]["position_end"], 4099)
        self.assertTrue(manifest["capture"]["fresh_process_bitwise_match"])
        self.assertFalse(
            manifest["capture"]["retained_prefill_payload"]["imported"]
        )
        self.assertEqual(manifest["selection"]["position_2048_selected_token"], 201)
        self.assertEqual(manifest["selection"]["position_4099_selected_token"], 2538)
    ...
    def test_prefill_layer3_ingress_fixture_manifest_and_payloads(self) -> None:
        manifest = json.loads(
            (PREFILL_LAYER3_INGRESS_FIXTURE / "manifest.json").read_text(
                encoding="utf-8"
            )
        )
        report = validate_differential_fixture(PREFILL_LAYER3_INGRESS_FIXTURE)
        self.assertEqual(
            report["fixture_id"],
        ...
        self.assertEqual(
            ...
            [[32, 4096], [32, 4096], [32, 1024]],
        )

    def test_prefill_layer3_kv_state_fixture_manifest_and_payloads(self) -> None:
        manifest = json.loads(
            (PREFILL_LAYER3_KV_STATE_FIXTURE / "manifest.json").read_text(
                encoding="utf-8"
            )
        )
        report = validate_differential_fixture(PREFILL_LAYER3_KV_STATE_FIXTURE)
        self.assertEqual(
            report["fixture_id"],
        ...
        self.assertEqual(report["operations"], 6)
        self.assertEqual(report["tensors"], 7)
        self.assertEqual(report["verified_bytes"], 8_781_824)
        self.assertEqual(manifest["scope"]["layer"], 3)
        self.assertEqual(manifest["scope"]["captured_position_range"], [2016, 2047])
