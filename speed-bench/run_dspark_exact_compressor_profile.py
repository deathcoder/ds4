...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Profile exact row-interleaved compression and indexing."
    )
...
def profile_env(layer=None):
    ...
    if layer is not None:
        ...
        env["DS4_DSPARK_EXACT_COMPRESSOR_PROFILE"] = "1"
    return env
...
def command_text(args, layer=None):
    ...
    keys = (
        ...
        "DS4_DSPARK_EXACT_COMPRESSOR_PROFILE",
    )
...
def parse_profile(data, expected_layer, path):
    ...
    for line in data.splitlines():
        ...
        if part not in ("exact", "compressor"):
            continue
        row = {
            "part": part,
            "layer": int(layer),
        ...
        rows.append(row)
    if not rows:
        raise RuntimeError(f"no exact-compressor profile records found in {path}")

    exact = [row for row in rows if row["part"] == "exact"]
    compressor = [row for row in rows if row["part"] == "compressor"]
    unknown_exact = sorted({row["stage"] for row in exact} - set(CONTROL_STAGES))
    unknown_compressor = sorted(
        {row["stage"] for row in compressor} - set(ALL_COMPRESSOR_STAGES)
    )
    if unknown_exact:
        raise RuntimeError(
            f"unknown exact control stages in {path}: {', '.join(unknown_exact)}"
        )
    if unknown_compressor:
        raise RuntimeError(
            f"unknown exact-compressor stages in {path}: "
            + ", ".join(unknown_compressor)
        )
...
def component_summary(rows, normalize_rows=False):
    values = [row["ms"] / row["tokens"] if normalize_rows else row["ms"] for row in rows]
...
def summarize(records):
    ...
    for layer in sorted({row["layer"] for row in records}):
        ...
        compressor = [row for row in selected if row["part"] == "compressor"]
        ...
        for stage in expected_stages:
            ...
            if sorted(row["pos"] for row in stage_rows) != expected_positions:
                raise RuntimeError(
        ...
        def split_update(stage):
            stage_rows = [row for row in compressor if row["stage"] == stage]
            nonemit = [row for row in stage_rows if (row["pos"] + 1) % ratio != 0]
            emit = [row for row in stage_rows if (row["pos"] + 1) % ratio == 0]
            return {
                "nonemit": component_summary(nonemit) if nonemit else None,
                "emit": component_summary(emit) if emit else None,
            }
...
def report(summary):
    lines = [
        "# DSpark Exact Compressor and Indexer Profile",
        "",
    ...
    for layer, item in summary["layers"].items():
        ...
        lines.append(
            ...
            f"{value(stages.get('indexer_projection'))} | "
            f"{value(index_update.get('nonemit'))} | "
            f"{value(index_update.get('emit'))} | "
            f"{value(stages.get('indexer_prepare'))} | "
            f"{value(stages.get('indexer_score'))} | "
            f"{value(stages.get('indexer_topk'))} |"
        )
...
def main():
    ...
    run_dir = (
        args.output_dir
        or root / "speed-bench/local-runs" / f"compressor-profile-{stamp}"
    ).resolve()
    ...
    metadata = {
        ...
        "config": {
            ...
            "compressor_components": ALL_COMPRESSOR_STAGES,
            "indexer_top_k": 512,
        },
    ...
    reference, _, reference_run = execute(args, root, run_dir, "reference", None, None)
    records = []
    runs = [reference_run]
    cooldown(args.cooldown)
    for layer in args.layers:
