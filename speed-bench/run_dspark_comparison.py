...
EXPERIMENT_ENV_KEYS = (
    ...
    "DS4_METAL_DENSE_MIXED_SPLIT_SOURCE",
    "DS4_METAL_DENSE_MIXED_VECTOR_PREPARE",
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "compare the prepared dense-mixed split-K vector against direct "
            "raw/compressed source loads"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "compare scalar dense-mixed preparation against a row-first "
            "half4 prepare kernel"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "compare the default exact router-weight path against the "
            "upstream batched weight-finalization kernel"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "compare default exact DSpark against a single Q2_K routed-down "
            "dispatch across verifier rows"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "compare default exact DSpark against paired routed gate/up at "
            "verifier width 5"
        ),
    )
    parser.add_argument(
    ...
    selected_modes = sum(
        ...
        for value in (
            ...
            args.dense_mixed_split_source_ablation,
            args.dense_mixed_vector_prepare_ablation,
            args.router_weights_batch_fusion_ablation,
    ...
    if selected_modes > 1:
        parser.error(
            ...
            "--dense-mixed-split-source-ablation, and "
            "--dense-mixed-vector-prepare-ablation, and "
            "--router-weights-batch-fusion-ablation, and "
            "--exact-routed-moe-hybrid-ablation "
...
def benchmark_modes(args):
    ...
    if getattr(args, "dense_mixed_vector_prepare_ablation", False):
        return ("default_exact", "dense_mixed_vector_prepare")
    if getattr(args, "dense_mixed_split_source_ablation", False):
        return ("default_exact", "dense_mixed_split_source")
...
def mode_label(mode, args):
    ...
    return {
        ...
        "dense_mixed_split_source": "Dense-mixed split-source DSpark",
        "dense_mixed_vector_prepare": "Dense-mixed vector-prepare DSpark",
        "router_weights_batch_fusion":
            "Batched router-weight finalization DSpark",
...
def throughput_runtime_stats_enabled(args):
    ...
    return not (
        ...
        or getattr(args, "dense_mixed_split_source_ablation", False)
        or getattr(args, "dense_mixed_vector_prepare_ablation", False)
        or getattr(args, "router_weights_batch_fusion_ablation", False)
...
def clean_dspark_env(mode, fast_verifier=False, runtime_stats=True):
    ...
    if mode != "baseline":
        ...
        if mode == "dense_mixed_split_source":
            env["DS4_METAL_DENSE_MIXED_SPLIT_SOURCE"] = "1"
        if mode == "dense_mixed_vector_prepare":
            env["DS4_METAL_DENSE_MIXED_VECTOR_PREPARE"] = "1"
        if mode == "router_weights_batch_fusion":
            env["DS4_METAL_ENABLE_ROUTER_WEIGHTS_BATCH_FUSION"] = "1"
...
def mode_command(args, mode):
    ...
    if (args.attention_pre_ablation or args.attention_suffix_ablation or
            ...
            getattr(args, "dense_mixed_split_source_ablation", False) or
            getattr(args, "dense_mixed_vector_prepare_ablation", False) or
            getattr(args, "router_weights_batch_fusion_ablation", False) or
...
def collect_metadata(args, root):
    ...
    return {
        ...
        "config": {
            ...
            "dense_mixed_split_source_ablation":
                getattr(args, "dense_mixed_split_source_ablation", False),
            "dense_mixed_vector_prepare_ablation":
                getattr(args, "dense_mixed_vector_prepare_ablation", False),
            "router_weights_batch_fusion_ablation":
                getattr(args, "router_weights_batch_fusion_ablation", False),
...
def summarize(rows, modes=("baseline", "runtime")):
    ...
    if modes == ("default_exact", "attention_output_nr8"):
        summary.update({
            "promoted_attention_output_nr4_generation_tps_median":
                reference_median,
    ...
    if modes == ("default_exact", "dense_mixed_split_source"):
        summary.update({
            "default_exact_generation_tps_median": reference_median,
            "dense_mixed_split_source_generation_tps_median":
                candidate_median,
            "dense_mixed_split_source_delta_percent":
                (candidate_median / reference_median - 1.0) * 100.0,
        })
        return summary

    if modes == ("default_exact", "dense_mixed_vector_prepare"):
        summary.update({
            "scalar_prepare_generation_tps_median": reference_median,
            "vector_prepare_generation_tps_median": candidate_median,
            "vector_prepare_delta_percent":
                (candidate_median / reference_median - 1.0) * 100.0,
        })
        return summary

    if modes == ("default_exact", "router_weights_batch_fusion"):
        summary.update({
            "default_exact_generation_tps_median": reference_median,
            "router_weights_batch_fusion_generation_tps_median":
                candidate_median,
            "router_weights_batch_fusion_delta_percent":
                (candidate_median / reference_median - 1.0) * 100.0,
        })
        return summary
    ...
    if modes == ("default_exact", "exact_q2_down_batch"):
        summary.update({
            "default_exact_generation_tps_median": reference_median,
    ...
    if modes == ("default_exact", "exact_routed_gate_up_pair_w5"):
        summary.update({
            "default_exact_generation_tps_median": reference_median,
...
def format_report(summary):
    ...
    if summary["comparison"] == "exact_attention_output_nr8_ablation":
        return (
            "# DSpark Exact Attention-Output NR8 Ablation\n\n"
            f"- Promoted NR4 median: "
            f"{summary['promoted_attention_output_nr4_generation_tps_median']:.2f} t/s\n"
            f"- Opt-in NR8 median: "
            f"{summary['attention_output_nr8_generation_tps_median']:.2f} t/s\n"
            f"- Ratio of medians: {summary['median_ratio_of_medians']:.4f}x\n"
            f"- Median paired ratio: {summary['paired_speedup_median']:.4f}x\n"
            f"- NR8 delta: "
    ...
    if summary["comparison"] == "dense_mixed_split_source_ablation":
        return (
            "# DSpark Dense-Mixed Split-Source Ablation\n\n"
            f"- Prepared split-K median: "
            f"{summary['default_exact_generation_tps_median']:.2f} t/s\n"
            f"- Split-source median: "
            f"{summary['dense_mixed_split_source_generation_tps_median']:.2f} t/s\n"
            f"- Ratio of medians: {summary['median_ratio_of_medians']:.4f}x\n"
            f"- Median paired ratio: {summary['paired_speedup_median']:.4f}x\n"
            f"- Split-source delta: "
            f"{summary['dense_mixed_split_source_delta_percent']:+.1f}%\n"
            f"- Measured pairs: {len(summary['paired_speedup_values'])}\n"
        )

    if summary["comparison"] == "dense_mixed_vector_prepare_ablation":
        return (
            "# DSpark Dense-Mixed Vector Prepare Ablation\n\n"
            f"- Scalar prepare median: "
            f"{summary['scalar_prepare_generation_tps_median']:.2f} t/s\n"
            f"- Vector prepare median: "
            f"{summary['vector_prepare_generation_tps_median']:.2f} t/s\n"
            f"- Ratio of medians: {summary['median_ratio_of_medians']:.4f}x\n"
            f"- Median paired ratio: {summary['paired_speedup_median']:.4f}x\n"
            f"- Vector prepare delta: "
            f"{summary['vector_prepare_delta_percent']:+.1f}%\n"
            f"- Measured pairs: {len(summary['paired_speedup_values'])}\n"
        )

    if summary["comparison"] == "router_weights_batch_fusion_ablation":
        return (
            "# DSpark Router-Weight Batch Fusion Ablation\n\n"
            f"- Default exact median: "
            f"{summary['default_exact_generation_tps_median']:.2f} t/s\n"
            f"- Batch-fused router weights median: "
            f"{summary['router_weights_batch_fusion_generation_tps_median']:.2f} t/s\n"
            f"- Ratio of medians: {summary['median_ratio_of_medians']:.4f}x\n"
            f"- Median paired ratio: {summary['paired_speedup_median']:.4f}x\n"
            f"- Batch-fusion delta: "
            f"{summary['router_weights_batch_fusion_delta_percent']:+.1f}%\n"
            f"- Measured pairs: {len(summary['paired_speedup_values'])}\n"
        )
    ...
    if summary["comparison"] == "exact_q2_down_batch_ablation":
        return (
            "# DSpark Exact Q2 Down Single-Dispatch Ablation\n\n"
            f"- Default exact median: "
            f"{summary['default_exact_generation_tps_median']:.2f} t/s\n"
            f"- Single-dispatch Q2 down median: "
            f"{summary['exact_q2_down_batch_generation_tps_median']:.2f} t/s\n"
            f"- Ratio of medians: {summary['median_ratio_of_medians']:.4f}x\n"
            f"- Median paired ratio: {summary['paired_speedup_median']:.4f}x\n"
    ...
    if summary["comparison"] == "exact_routed_gate_up_pair_w5_ablation":
        return (
            "# DSpark Exact Routed Gate/Up Pair Width-5 Ablation\n\n"
            f"- Default exact median: "
            f"{summary['default_exact_generation_tps_median']:.2f} t/s\n"
            f"- Width-5 paired gate/up median: "
            f"{summary['exact_routed_gate_up_pair_w5_generation_tps_median']:.2f} t/s\n"
            f"- Ratio of medians: {summary['median_ratio_of_medians']:.4f}x\n"
            f"- Median paired ratio: {summary['paired_speedup_median']:.4f}x\n"
