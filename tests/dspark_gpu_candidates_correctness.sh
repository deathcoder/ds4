...
if [[ $dense_mixed_vector_prepare == 1 &&
    ...
    dense_mixed_split_source_parity=1
fi
...
unset DS4_DSPARK_EXACT_COMPRESSOR_PRE_BATCH_TRACE
...
if [[ $dense_mixed_vector_prepare != 0 &&
      $dense_mixed_vector_prepare != 1 ]]; then
    printf 'DS4_TEST_DSPARK_DENSE_MIXED_VECTOR_PREPARE must be 0 or 1\n' >&2
    exit 2
fi
if [[ $dense_mixed_vector_prepare == 1 &&
      $dense_mixed_split_source == 1 ]]; then
    printf 'dense-mixed vector prepare and split source are mutually exclusive\n' >&2
    exit 2
fi
if [[ $dense_mixed_vector_prepare == 1 &&
      $dense_mixed_gathered_legacy == 1 ]]; then
    printf 'dense-mixed vector prepare requires the fused-gather route\n' >&2
    exit 2
fi
if [[ $dense_mixed_vector_prepare == 1 &&
      ($mode != runtime || $fast_verify_runtime == 1) ]]; then
    printf 'dense-mixed vector prepare requires exact runtime verification\n' >&2
    exit 2
fi
if [[ $dense_mixed_split_source == 1 &&
      ($mode != runtime || $fast_verify_runtime == 1) ]]; then
...
if [[ $dense_mixed_split_source_parity == 1 &&
      $dense_mixed_gathered_legacy == 1 ]]; then
    printf 'dense-mixed split-source parity requires the fused-gather route\n' >&2
    exit 2
fi
if [[ $dense_mixed_split_source_parity == 1 &&
      ($mode != runtime || $fast_verify_runtime == 1) ]]; then
    printf 'dense-mixed split-source parity requires exact runtime verification\n' >&2
    exit 2
fi
...
case "$mode" in
    ...
    runtime)
        ...
        if [[ $dense_mixed_vector_prepare == 1 ]]; then
            gpu_env+=(
...
if [[ $exact_compressor_pre_batch == 1 ]]; then
    if ! grep -q 'DSpark exact compressor prebatch .* ratio=128 result=ok$' \
        "$tmpdir"/*.log ||
       ! grep -q 'DSpark exact compressor prebatch .* ratio=4 result=ok$' \
        "$tmpdir"/*.log; then
        printf 'exact compressor prebatch did not engage both compression ratios\n' >&2
        exit 1
    fi
    if grep -q 'DSpark exact compressor prebatch .* result=fallback$' \
        "$tmpdir"/*.log; then
        printf 'exact compressor prebatch fell back\n' >&2
        exit 1
    fi
fi
...
if [[ $dense_mixed_split_source_parity == 1 ]]; then
    if ! grep -q \
        'Metal dense mixed split-source parity .* result=ok$' \
        "$tmpdir"/*.log; then
        printf 'dense-mixed split-source parity did not engage\n' >&2
        exit 1
    fi
    if grep -Eq \
        'Metal dense mixed split-source parity .* result=(mismatch|command_error)$' \
        "$tmpdir"/*.log; then
        printf 'dense-mixed split-source parity failed\n' >&2
        exit 1
    fi
    if ! grep -Eq \
        'Metal dense mixed split-source parity .* raw_start=[1-9][0-9]* .* result=ok$' \
        "$tmpdir"/*.log; then
