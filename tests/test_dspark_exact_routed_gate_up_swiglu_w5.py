#!/usr/bin/env python3
"""Model-free tests for fused routed gate/up SwiGLU at verifier width 5."""
...
def args():
    return SimpleNamespace(
        ...
        exact_routed_gate_up_pair_w5_ablation=False,
        exact_routed_gate_up_swiglu_w5_ablation=True,
        stats_only=False,
    )


class ExactRoutedGateUpSwiGLUW5Tests(unittest.TestCase):
    def test_ablation_compares_default_with_width5_fusion(self):
        self.assertEqual(
    ...
    def test_only_candidate_enables_width5_fusion(self):
        reference = comparison.clean_dspark_env(
            "default_exact", runtime_stats=False
        )
        candidate = comparison.clean_dspark_env(
            "exact_routed_gate_up_swiglu_w5", runtime_stats=False
        )
        key = "DS4_DSPARK_EXACT_ROUTED_GATE_UP_SWIGLU_W5"
    ...
    def test_commands_are_metal_and_uninstrumented(self):
        ...
        candidate = comparison.command_text(
            args(), "exact_routed_gate_up_swiglu_w5", runtime_stats=False
        )
        self.assertNotIn("EXACT_ROUTED_GATE_UP_SWIGLU_W5", reference)
        self.assertIn(
            "DS4_DSPARK_EXACT_ROUTED_GATE_UP_SWIGLU_W5=1", candidate
        )
    ...
    def test_summary_reports_paired_gain(self):
        rows = [
            ...
            {
                "pair": 1,
                "mode": "exact_routed_gate_up_swiglu_w5",
                "generation_tps": 11.0,
            },
            {
                "pair": 2,
        ...
        summary = comparison.summarize(
            rows, ("default_exact", "exact_routed_gate_up_swiglu_w5")
        )
        self.assertEqual(
            summary["comparison"],
        ...
        self.assertIn("Routed Gate/Up SwiGLU Width-5 Ablation", report)
        self.assertIn("Width-5 fused gate/up SwiGLU delta", report)

    def test_candidate_fuses_activation_only_on_exact_width5(self):
        source = (ROOT / "ds4_metal.m").read_text(encoding="utf-8")
        self.assertIn("const bool exact_routed_gate_up_swiglu_w5", source)
        self.assertIn("exact_down_rows &&", source)
        self.assertIn("n_tokens == 5u", source)
        self.assertIn(
            "g_moe_mul_mv_id_iq2_xxs_pair_swiglu_pipeline", source
        )
        self.assertIn(
    ...
    def test_correctness_matrix_requires_successful_candidate_trace(self):
        ...
        self.assertIn(
            "DS4_TEST_DSPARK_EXACT_ROUTED_GATE_UP_SWIGLU_W5", source
        )
        self.assertIn(
            "DS4_DSPARK_EXACT_ROUTED_GATE_UP_SWIGLU_W5_TRACE=1", source
        )
        self.assertIn("DSpark exact routed gate/up SwiGLU width=5", source)
