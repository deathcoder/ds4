...
class ExactAttentionOutputNr4Tests(unittest.TestCase):
    ...
    def test_only_candidate_enables_nr4(self):
        reference = comparison.clean_dspark_env(
            "default_exact", runtime_stats=False
        )
        candidate = comparison.clean_dspark_env(
            "exact_attention_output_nr4", runtime_stats=False
        )
        self.assertNotIn("DS4_DSPARK_EXACT_ATTN_OUT_NR4", reference)
        self.assertEqual(candidate["DS4_DSPARK_EXACT_ATTN_OUT_NR4"], "1")
        self.assertNotIn("DS4_DSPARK_EXACT_ATTN_OUT_NR4_TRACE", candidate)
    ...
    def test_commands_are_metal_and_uninstrumented(self):
        ...
        candidate = comparison.command_text(
            args(), "exact_attention_output_nr4", runtime_stats=False
        )
        self.assertNotIn("DS4_DSPARK_EXACT_ATTN_OUT_NR4", reference)
        self.assertIn("DS4_DSPARK_EXACT_ATTN_OUT_NR4=1", candidate)
    ...
    def test_summary_reports_paired_gain(self):
        rows = [
            ...
            {
                "pair": 1,
                "mode": "exact_attention_output_nr4",
                "generation_tps": 11.0,
            },
            {
                "pair": 2,
        ...
        summary = comparison.summarize(
            rows, ("default_exact", "exact_attention_output_nr4")
        )
        self.assertEqual(
        ...
        self.assertIn("Exact Attention-Output NR4 Ablation", report)
        self.assertIn("NR4 attention-output delta", report)
    ...
    def test_kernels_preserve_q8_reduction_shape(self):
        ...
        self.assertIn(
            'getenv("DS4_DSPARK_EXACT_ATTN_OUT_NR4")', metal_host
        )
        self.assertIn(
