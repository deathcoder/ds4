#!/usr/bin/env python3
"""Model-free tests for the exact attention-output NR8 gate."""
...
def args():
    return SimpleNamespace(
        ...
        exact_attention_output_nr8_ablation=True,
        stats_only=False,
    )


class ExactAttentionOutputNr8Tests(unittest.TestCase):
    def test_ablation_compares_promoted_nr4_with_nr8(self):
        self.assertEqual(
    ...
    def test_only_candidate_enables_nr8(self):
        reference = comparison.clean_dspark_env(
            "default_exact", runtime_stats=False
        )
        candidate = comparison.clean_dspark_env(
            "attention_output_nr8", runtime_stats=False
        )
        self.assertNotIn("DS4_DSPARK_EXACT_ATTN_OUT_NR8", reference)
        self.assertEqual(candidate["DS4_DSPARK_EXACT_ATTN_OUT_NR8"], "1")
        self.assertNotIn("DS4_DSPARK_EXACT_ATTN_OUT_NR8_TRACE", candidate)
    ...
    def test_commands_are_metal_and_uninstrumented(self):
        ...
        candidate = comparison.command_text(
            args(), "attention_output_nr8", runtime_stats=False
        )
        self.assertNotIn("DS4_DSPARK_EXACT_ATTN_OUT_NR8", reference)
        self.assertIn("DS4_DSPARK_EXACT_ATTN_OUT_NR8=1", candidate)
    ...
    def test_summary_reports_paired_gain(self):
        rows = [
            ...
            {"pair": 1, "mode": "attention_output_nr8", "generation_tps": 11.0},
            {"pair": 2, "mode": "attention_output_nr8", "generation_tps": 12.0},
            {"pair": 2, "mode": "default_exact", "generation_tps": 10.0},
        ]
        summary = comparison.summarize(
            rows, ("default_exact", "attention_output_nr8")
        )
        self.assertEqual(
            summary["comparison"], "exact_attention_output_nr8_ablation"
        )
        self.assertAlmostEqual(summary["paired_speedup_median"], 1.15)
        report = comparison.format_report(summary)
        self.assertIn("NR8 Ablation", report)
        self.assertIn("NR8 delta", report)
    ...
    def test_kernels_preserve_q8_reduction_shape(self):
        ...
        self.assertIn(
            'ds4_gpu_env_bool("DS4_DSPARK_EXACT_ATTN_OUT_NR8")',
            metal_host,
        )
        self.assertIn(
            "kernel_dsv4_attn_out_low_q8_0_f32_impl<8>", moe
        )
        self.assertIn(
            "kernel_dsv4_q8_hc_expand4_q8_0_impl<8>", hc
        )
