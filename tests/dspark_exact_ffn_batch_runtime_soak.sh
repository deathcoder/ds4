...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/ds4-exact-ffn-runtime.XXXXXX")
cleanup() {
    local status=$?
...
assert_candidate_log() {
    ...
    if [[ $(grep -c 'DSpark exact FFN batch runtime .* result=pass' "$log") -lt $min_calls ]]; then
        printf 'exact FFN batch runtime did not run enough times for %s\n' "$name" >&2
        exit 1
    fi
    if grep -Eq 'DSpark exact FFN batch runtime .* result=fail|DSpark batch capture skipped|DSpark GPU candidate source fallback' "$log"; then
        printf 'exact FFN batch runtime invariant failed for %s\n' "$name" >&2
        exit 1
    fi
}
...
compare_prompt() {
    ...
    printf 'PASS exact FFN runtime soak: %s\n' "$name"
}
...
compare_resumed_chat() {
    ...
    ' "$runtime_log"; then
        printf 'exact FFN batch runtime did not verify after resumed sync\n' >&2
        exit 1
    fi
    printf 'PASS exact FFN runtime soak: resumed_chat\n'
}
...
printf 'DSpark exact FFN batch runtime correctness soak: PASS\n'
