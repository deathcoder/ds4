...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/ds4-exact-attn-suffix-runtime.XXXXXX")
cleanup() {
    local status=$?
...
runtime_env=(
    ...
    DS4_DSPARK_EXACT_ATTN_SUFFIX_BATCH=1
...
assert_candidate_log() {
    ...
    local min_calls=$3
    local records
    records=$(grep 'DSpark exact attention suffix batch runtime proposed=' "$log")
    if printf '%s\n' "$records" |
        grep -Evq ' layers=43 attempts=43 successes=43 result=pass$'; then
        printf 'exact attention-suffix runtime drifted or fell back for %s\n' "$name" >&2
        printf '%s\n' "$records" >&2
        exit 1
    fi
    if [[ $(printf '%s\n' "$records" | wc -l | tr -d ' ') -lt $min_calls ]]; then
        printf 'exact attention-suffix runtime did not run enough times for %s\n' "$name" >&2
        exit 1
    fi
    grep -q 'DSpark exact batch verifier .* result=pass' "$log"
    grep -q 'DSpark GPU candidate source selected' "$log"
    if grep -Eq 'DSpark exact attention suffix batch runtime .* result=(fail|fallback)|DSpark batch capture skipped|DSpark GPU candidate source fallback' "$log"; then
        printf 'exact attention-suffix runtime invariant failed for %s\n' "$name" >&2
        exit 1
    fi
}
...
compare_prompt() {
    ...
    printf 'PASS exact attention-suffix runtime soak: %s\n' "$name"
}
...
compare_resumed_chat() {
    ...
    if ! awk '
        /DSpark exact attention suffix batch runtime retained after resumed sync/ {
            resumed = 1
            next
        }
        resumed && /DSpark exact attention suffix batch runtime .* result=pass/ {
            verified = 1
        }
        END { exit verified ? 0 : 1 }
    ' "$runtime_log"; then
        printf 'exact attention-suffix runtime did not verify after resumed sync\n' >&2
        exit 1
    fi
    printf 'PASS exact attention-suffix runtime soak: resumed_chat\n'
}

compare_serial_fallback() {
    local control_out="$tmpdir/fallback.control.out"
    local runtime_out="$tmpdir/fallback.runtime.out"
    local control_log="$tmpdir/fallback.control.log"
    local runtime_log="$tmpdir/fallback.runtime.log"
    local prompt_file="$root/speed-bench/dspark_prompt.txt"
    local control_env=(
        DS4_DSPARK_GPU_RUNTIME=1
        DS4_DSPARK_MULTI_COMMIT=1
        DS4_DSPARK_FAST_BATCH_VERIFY=0
    ...
    env "${control_env[@]}" \
        "$ds4_bin" "${common[@]}" --dspark "$dspark_model" \
        -n 12 --prompt-file "$prompt_file" >"$control_out" 2>"$control_log"
    env "${runtime_env[@]}" DS4_METAL_DISABLE_ATTN_OUT_HC_FUSION=1 \
        "$ds4_bin" "${common[@]}" --dspark "$dspark_model" \
        -n 12 --prompt-file "$prompt_file" >"$runtime_out" 2>"$runtime_log"

    if ! cmp -s "$control_out" "$runtime_out"; then
        printf 'stdout mismatch for serial attention-suffix fallback\n' >&2
        diff -u "$control_out" "$runtime_out" >&2 || true
        exit 1
    fi
    local records
    records=$(grep 'DSpark exact attention suffix batch runtime proposed=' "$runtime_log")
    if printf '%s\n' "$records" |
        grep -Evq ' layers=43 attempts=43 successes=0 result=fallback$'; then
        printf 'exact attention-suffix serial fallback was not exercised cleanly\n' >&2
        printf '%s\n' "$records" >&2
        exit 1
    fi
    grep -q 'DSpark exact batch verifier .* result=pass' "$runtime_log"
    printf 'PASS exact attention-suffix runtime soak: serial_fallback\n'
}
...
printf 'DSpark exact attention-suffix batch runtime correctness soak: PASS\n'
