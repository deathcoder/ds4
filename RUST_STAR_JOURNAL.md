...
- Implementation: dependency-free Rust host scaffold under `rust-star/runtime/`
  ...
  All retained layer-7 through layer-36 boundaries, full
  attention outputs, compressor states, and full HC
  identities match fresh DwarfStar processes exactly, establishing complete
  native layers 0–36 at the same prompt boundary with 1,036/1,036 no-copy
  mappings across 2,021 terminal dispatches. A separate
...
1. Continue the exact batched-prefill frontier through layer 37 while preserving
   every retained layers-0–36 boundary as a regression control.
